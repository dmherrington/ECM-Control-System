...
void CommsMarshaler::AddProtocol(const MavlinkConfiguration &config)
{
    ...
    std::shared_ptr<GalilProtocol> protocol = std::make_shared<GalilProtocol>(config);
...
void CommsMarshaler::SetProtocolForLink(const std::string &linkName, Protocols protocol)
{
    ...
    if(protocol == Protocols::MAVLINK)
    {
        ...
        std::shared_ptr<GalilProtocol> mavlinkProtocol = std::static_pointer_cast<GalilProtocol>(protocolObj);
...
//!
template <typename T>
void CommsMarshaler::SendGalilMessage(const std::string &linkName, const T& message)
{
    std::shared_ptr<ILink> link = m_CreatedLinksNameToPtr.at(linkName);
    ...
    auto func = [protocol_code, protocol_obj, link, message]() {
        ...
        {
        case Protocols::GALIL:
        {
            std::shared_ptr<GalilProtocol> protocol = std::static_pointer_cast<GalilProtocol>(protocol_obj);
